...
export default function WordsList({
  ...
  return (
    <div className="overflow-y-auto max-h-[420px]">
      {words.map((word) => {
        const status = statusConfig[word.status] ?? { label: word.status, className: "bg-slate-100 text-slate-500 border-slate-200" };
        return (
          <div
            key={word.id}
          ...
          >
            ...
            <span
              className={`inline-flex w-fit items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${status.className}`}
            >
              {status.label}
            </span>
